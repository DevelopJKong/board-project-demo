...
export const postCertification = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { name: _name, birth: _birth, phone } = certificationsInfo;

    const user = await User.findOne({ phoneNum: phone });

    if (user) {
      logger.error(`이미 존재하는 핸드폰 번호 입니다`);
      return res.send({
        status: 'fail',
        message: '이미 존재하는 핸드폰 번호 입니다',
      });
    }
    logger.info(`인증이 완료 되었습니다`);
    return res.send({
      status: 'success',
    ...
    // ! 500 error
    logger.error(`postCertification error`);
    console.log(error);
  }
};

export const getCheck = (req: Request, res: Response) => {
  //* 200 success
  logger.info(`check.pug 랜더 성공`);
  return res.render('check');
};

export const postCheck = async (req: Request, res: Response) => {
...
export const postJoin = async (req: Request, res: Response) => {
  try {
    ...
    const mailVar = {
      ...
      <p style="color:#0984e3; font-size: 25px;">${splittedCodeNum}</p>
      <br/>
    ...
    await Verification.create({
      code: splittedCodeNum,
      user,
    });

    await sendMailer(mailVar);
    //* 200 success 성공
    logger.info(`check 랜더 성공 & 데이터 user 전달 성공`);
...
export const postLogin = async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      ...
      logger.error(`아이디 / 비밀번호 가 틀렸습니다 ❌`);
      return res.status(401).render('login', { errorMessage: '아이디 / 비밀번호 가 틀렸습니다 ❌' });
    }
    const validationCheck = bcrypt.compare(password, user?.password ?? '');
    ...
    if (!user.verified) {
      //* 200 success
      logger.info(`login 랜더 성공 & 데이터 user 전달 성공`);
      return res.render('check', { user });
    }
...
export const getEdit = (req: Request, res: Response) => {
  //* 200 success
  logger.info(`edit-profile 랜더 성공`);
...
export const postEdit = async (req: Request, res: Response) => {
  ...
  const searchParam = [] as Array<Record<string, string>>;
  if (sessionEmail !== email) {
    searchParam.push({ email: email as string });
  }
  if (sessionUsername !== username) {
    searchParam.push({ username: username as string });
  }
  if (searchParam.length > 0) {
    const foundUser = await User.findOne({ $or: searchParam });
    if (foundUser && foundUser._id.toString() !== _id) {
      // ! 401 error 이미 존재 하는 계정이 있는 경우
      logger.error(`이미 있는 아아디나 이메일 입니다 ❌`);
  ...
  const updateUser = await User.findByIdAndUpdate(
    _id,
    {
      avatar: file ? (isHeroku ? (file as any).location : file.path) : avatar,
      name,
      email,
      username,
      region,
...
export const postChangePassword = async (req: Request, res: Response) => {
  ...
  //* 304 redirect
  logger.info(`"users/logout" logout.pug로 리다이렉트 성공`);
...
export const naverLogin = (req: Request, res: Response) => {
  ...
  const { client_id, client_secret: _client_secret, state, redirectURI } = config;
  ...
  //* 304 redirect
  logger.info(`${api_url}으로 리다이렉트 성공`);
...
export const naverCallback = async (req: Request, res: Response) => {
  ...
  if ('access_token' in tokenRequest) {
    ...
    if (existingUser) {
      ...
      logger.error(`존재하는 계정입니다 일반 로그인으로 로그인 해주세요 ❌`);
